...
| pregnancy_outcome | enum(`persalinan`,`keguguran`), nullable | Cara kehamilan berakhir; wajib diisi saat profil pertama kali keluar dari status `hamil` |
| ended_at | timestamp, nullable | Waktu kehamilan berakhir; diisi saat profil pertama kali keluar dari status `hamil` |
...
| weight_kg | decimal(5,2), nullable | Pengukuran medis memakai Decimal, bukan Float, untuk menjaga presisi desimal. |
| fundal_height_cm | decimal(5,2), nullable | Pengukuran medis memakai Decimal, bukan Float, untuk menjaga presisi desimal. |
| protein_urine | varchar, nullable | |
| platelet_count | decimal(10,2), nullable | Pengukuran medis memakai Decimal, bukan Float, untuk menjaga presisi desimal. |
| recorded_at | timestamp | |
| client_uuid | uuid, nullable | Buat idempotency sync offline |
| created_at | timestamp | |

Field pengukuran medis di atas sengaja memakai tipe fixed-precision `Decimal`, bukan floating-point, agar nilai klinis tidak terkena kesalahan representasi biner.
...
| id | uuid, PK | |
| pregnancy_profile_id | uuid, FK | |
| checkin_type | enum(`pregnancy`) | Hanya untuk skrining masa kehamilan. |
| answers | jsonb | |
...
> Data masa nifas tidak boleh ditulis ke `symptom_checkins`. Seluruh check-in
> postpartum eksklusif memakai tabel `postpartum_logs` dan endpoint
> `POST /postpartum-logs` agar rule engine menerima field klinis terstruktur.
...
| evaluation_reason | text, nullable | Alasan hasil evaluasi AI Service |
| mental_health_flag | boolean, nullable | Flag kesehatan mental dari AI Service |
| evaluated_at | timestamp, nullable | Waktu evaluasi synchronous/callback selesai |
| client_uuid | uuid, nullable, unique | Idempotensi input langsung dan mode kader offline |
| created_at | timestamp | |

Constraint/index tambahan dari migrasi Task 09:
- unique index pada `client_uuid` untuk mencegah duplikasi retry/offline;
- composite index `(pregnancy_profile_id, day_number)` untuk histori urut hari;
- composite index `(pregnancy_profile_id, created_at)` untuk histori terbaru.
...
Terdapat unique constraint `(pregnancy_profile_id, reminder_type)` agar
cadence setiap jenis reminder dapat di-upsert secara atomik.
...
| PATCH | `/pregnancy-profiles/:id/status` | bidan, kader | Ubah status ke `nifas`/`selesai`; sertakan `pregnancy_outcome` saat keluar dari `hamil` |
...
### 7.1 Lifecycle dan outcome kehamilan

`pregnancy_profiles.status` menyatakan fase lifecycle saat ini, sedangkan
`pregnancy_profiles.pregnancy_outcome` menyatakan cara kehamilan berakhir.
Keduanya tidak boleh dicampur menjadi satu enum.

Aturan transisi yang sudah pasti:

```text
hamil -> nifas -> selesai
hamil -> selesai, hanya dengan pregnancy_outcome = keguguran
```

- Saat profil pertama kali keluar dari `hamil`, `pregnancy_outcome` wajib diisi
  dengan `persalinan` atau `keguguran`, dan `ended_at` diisi waktu server.
- `hamil -> nifas` dengan outcome `persalinan` adalah jalur persalinan.
- `nifas -> selesai` tidak boleh mengubah `pregnancy_outcome` atau `ended_at`.
- Status dan outcome tidak boleh dimundurkan atau diganti setelah kehamilan
  berakhir.
- Backend tidak boleh menghitung usia kehamilan lalu memilih jalur transisi
  secara otomatis.

> **TBD — wajib validasi manusia ke dr. Julian/nakes:** Untuk outcome
> `keguguran`, pemilihan `hamil -> nifas` (perlu monitoring lanjutan) versus
> `hamil -> selesai` (langsung ditutup) bersifat kondisional berdasarkan
> threshold medis yang belum ditentukan. Sampai ada konfirmasi, endpoint hanya
> menjalankan jalur yang dipilih eksplisit oleh petugas berwenang; backend tidak
> mengimplementasikan threshold atau inferensi medis tetap.

Dampak fitur turunan:

- Reminder kehamilan/ANC harus dihentikan ketika profil keluar dari `hamil`.
- Reminder dan Postpartum Tracker untuk outcome `keguguran` belum boleh
  diaktifkan atau dilewati otomatis berdasarkan usia kehamilan. Perilakunya
  mengikuti status eksplisit (`nifas` atau `selesai`) sambil menunggu validasi
  medis di atas.

### 7.2 Risk badge (nilai enum, bukan cara hitungnya)
...
### 7.3 Cadence reminder (dihitung & dijadwalkan NestJS lewat BullMQ)
...
- Nilai Prisma `Decimal` seperti `weight_kg`, `fundal_height_cm`, dan `platelet_count` diserialisasi sebagai string di JSON. Kode yang melakukan kalkulasi numerik—terutama trend-based early warning—**WAJIB** parse eksplisit ke `number` dan memvalidasi hasilnya sebelum dihitung; dilarang mengandalkan coercion implisit JavaScript.

### 10.5 Konvensi HTTP Status Code
- `201 Created` hanya dipakai ketika request berhasil membuat resource baru.
- `200 OK` dipakai untuk operasi sukses yang tidak membuat resource baru, termasuk autentikasi/login dan idempotent replay yang mengembalikan resource yang sudah ada.
- Endpoint idempotent wajib membedakan insert baru (`201`) dari replay (`200`) tanpa mengubah bentuk payload resource.

### 10.6 Checklist sebelum fitur dianggap selesai
