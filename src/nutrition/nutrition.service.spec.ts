...
describe('NutritionService', () => {
  ...
  it('acknowledges a confident nutrition report through WhatsApp', async () => {
      ...
      pregnancy_profiles: [
        {
          ...
          nutrition_prompt_window: {
            window_closes_at: new Date(Date.now() + 60_000),
          },
        },
      ],
    });
    ai.parseNutrition.mockResolvedValue({
      calories: 420,
      iron_mg: 3.5,
      activity: 'jalan 20 menit',
      confidence_score: 0.9,
    });
    prisma.nutritionActivityLog.create.mockResolvedValue({
      id: 'log-1',
      status: 'processed',
    });
    ...
    await expect(
      service.handleInbound(
        { sender: '6281328282288', message: dto.raw_message },
        'request-1',
      ),
    ).resolves.toEqual({
      status: 'processed',
  ...
  it('asks for clarification once when confidence is low', async () => {
    ...
    await service.handleInbound(
      { sender: '+6281328282288', message: 'makan biasa' },
      'request-2',
    );
  ...
  it('sends chat answer to WhatsApp outside prompt window', async () => {
    ...
    chat.sendTrustedWebhookMessage.mockResolvedValue({
      reply: 'Mual ringan cukup umum.',
    ...
    await expect(
      service.handleInbound(
        { sender: '081328282288', message: 'Apakah mual normal?' },
        'request-3',
      ),
    ).resolves.toEqual(
  ...
  it('never replies to an unknown sender', async () => {
    ...
    await expect(
      service.handleInbound(
        { sender: '6289999999999', message: 'halo' },
        'request-4',
      ),
  ...
  it('retries processing when the existing provider message log failed', async () => {
    prisma.nutritionActivityLog.findUnique.mockResolvedValue({
      id: 'failed-log',
      status: 'failed',
    });
    ...
    await expect(
      service.handleInbound(
        {
          sender: '6281328282288',
          message: dto.raw_message,
          inboxid: 208625777,
        },
        'request-retry',
      ),
    ...
    expect(prisma.nutritionActivityLog.delete).toHaveBeenCalledWith({
      where: { id: 'failed-log' },
    });
  ...
  it('groups multiple meal entries into the same Jakarta daily log', async () => {
    ...
    await service.handleInbound(
      { sender: '6281328282288', message: 'sarapan nasi satu piring' },
      'daily-1',
    );
    await service.handleInbound(
      { sender: '6281328282288', message: 'makan siang nasi satu piring' },
      'daily-2',
    );
    ...
    expect(prisma.nutritionActivityLog.create).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
        data: expect.objectContaining({
          meal_period: 'breakfast',
          nutrition_daily_log_id: 'daily-log-1',
        }),
      }),
    );
    expect(prisma.nutritionActivityLog.create).toHaveBeenNthCalledWith(
