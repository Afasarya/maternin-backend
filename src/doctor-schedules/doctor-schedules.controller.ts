...
export class DoctorSchedulesController {
  ...
  @Put() async replace(
    ...
    const sorted = [...dto.schedules].sort(
      (a, b) =>
    ...
    const conflict = sorted.find(
      (item, index) =>
        index > 0 &&
        sorted[index - 1].day_of_week === item.day_of_week &&
        item.start_time < sorted[index - 1].end_time,
    );
    if (conflict)
      throw new BadRequestException(
        `Jadwal overlap pada ${conflict.day_of_week}: ${conflict.start_time}-${conflict.end_time}`,
      );
    const doctor = await this.doctors.findByUser(user.id);
